...
// class for AIFF audio file reading and writing
...
#include "aflibAiffFile.h"
#include "aflibData.h"

#include "aflibFileItem.h"

#define MODULE_NAME "aflibAiffFile"

extern "C"
{
   aflibFile *
   getAFileObject() { return ((aflibFile *)new aflibAiffFile()); }
	...
	query(list<aflibFileItem*>& support_list)
	{
		...
		item->setExtension(".aiff");
		item->setName(MODULE_NAME);
		item->setMagic("0(F), 1(O), 2(R), 3(M), 8(A), 9(I), 10(F), 11(F)");
   ...
   aflibConfig* cfg)
{
   ...
   // Set the input and output audio config data
   ...
   const aflibConfig& cfg)
{
   ...
   // Set output and input config data
   setInputConfig(cfg);
...
bool
aflibAiffFile::isDataSizeSupported(aflib_data_size size)
{
   int sampfmt, sampwidth;
...
bool
aflibAiffFile::isEndianSupported(aflib_data_endian end)
{
   int end_value;
...
bool
aflibAiffFile::isSampleRateSupported(int& rate)
{
   int value;
   double double_value;
   ...
   // If same then return TRUE else return desired rate
   if (value == rate)
      ret_value = TRUE;
   else
      rate = value;
